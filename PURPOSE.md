...
| File | Role |
|---|---|
| **`index.html`** | **The one page.** Everything the audience sees lives here, as tabs: Velocity, Critical Issues, AI Champions, Report, **Best Practices**, Conclusions. Switching tabs swaps the visible panel — there is no second page to keep in sync. |
| **`best-practices.html`** | **A redirect stub, not a page.** Best Practices was built as its own file on 2026-08-26, then folded into `index.html` as a sixth tab the same week — a two-file "hub + report" product read as two different sites even with cross-links between them, which wasn't the goal. This file now just forwards to `index.html#best-practices`, kept so any link already pointing at it still resolves. |
...
**`index.html` is the only page.** When something needs to be shown, it goes in `index.html`, as a tab or a section inside one. Do not create parallel or duplicate HTML pages — the Best Practices content has now drifted across three different homes in three weeks (an in-page pointer, a standalone redesigned file, and now a tab) precisely because each move seemed reasonable in isolation. It is a tab now, and that is meant to be the last move: everything the survey produces lives in one file with one nav.
...
| **Best Practices** | *The full content, as a tab* (since 2026-08-26). Its own sub-nav (Overview · Practice Library · Workflows · Champions · Examples · Resources), scoped under `#tab-best-practices` in the shared stylesheet so it can reuse names like `.card` without colliding with the rest of the report. Its own lighter Champions section (profile cards) is deliberately not the same as the full dossiers below — it links to the **Champions** tab (`data-goto="champions"`) for those. |
...
| 2026-08-26 | **`index.html`'s visual language brought in line with the Best Practices hub** — direct feedback that, nav aside, the report still looked like a different product. Added the same depth/shape system used on `best-practices.html` (soft shadows, rounded panel corners, pill buttons, a radial accent wash behind the masthead, larger section titles) to `.card`, `.insight`/`.alert`/`.why`, `.chp` (champion dossiers), `.track-count`, `.gobtn`, and `.story-step.final` — all via shared tokens (`--r-sm/md/lg`, `--sh-1/2/3`), so the change is centralized, not per-instance. **Deliberately left untouched:** the `:root` color values and the `.pill`/`.conf`/`.priority-*` tag shapes. The stylesheet's own opening comment documents that the two data hues (pine/vermillion) were validated for contrast and colorblind-safety against the exact panel surface color, and separately states tags are "square-cut... never a filled candy pill" — both are intentional, prior decisions this pass respects rather than overwrites. Chart.js color constants (TEAL/AMBER/GRAY/RED) are unchanged for the same reason |
| 2026-08-26 | **A shared top nav replaces each file's separate one** — direct feedback that the two-file split (however well cross-linked) read as "two different sites." Both files now carry the identical nav component: Cluster A (Velocity · Issues · Champions · Report · Conclusions — the survey report, cross-file links from `best-practices.html`, in-page tab switches from `index.html`) · a divider · Cluster B (Best Practices · Workflows · Champions · Examples · Resources — the hub, cross-file links from `index.html`, in-page anchors from `best-practices.html`). `index.html`'s now-redundant `tab-best-practices` pointer panel and its tab button are removed — the shared nav supersedes it, so there is exactly one way to reach the hub, not two. Every destination is one click away from either file |
...
| 2026-08-26 | **Best Practices folded back into `index.html`, this time as a real tab** — direct instruction, after the shared-nav pass, for one main page with tabs (Velocity/Issues/Champions/Report/Best Practices/Conclusions) where switching tabs swaps the content, not two files. The entire hub's markup, CSS and JS (hero, insights, maturity model, practice library with search/filter/bookmark, workflows, a lighter Champions view, examples, resources) moved into a sixth `.tab-panel`, given the same `01 · Section` sub-nav convention already used by the Report and Champions tabs. **Every hub-specific CSS selector is scoped under `#tab-best-practices`**, and the handful of its custom properties not already in this file's `:root` (`--surface`, `--ink`, `--accent`, etc.) are aliased to this file's own existing tokens inside that same scope — so the tab reuses the report's one validated palette instead of importing a second one. `best-practices.html` is now a redirect stub to `index.html#best-practices`, kept only so old links still resolve. **Two real bugs surfaced and were fixed during this merge, both the same failure mode:** an orphaned CSS block from an earlier iteration (`.workflow-card`, `.wf-title`, `.copy-btn`, `.do-header.do`, etc. — dead code no HTML referenced any more) was deleted outright; a second collision, `.maturity-seg`, belongs to a *still-live* proportion-bar component elsewhere in the report, so it couldn't be deleted — its scoped override now explicitly resets every property the live rule sets (`display`, `align-items`, `font-*`, `color`, `transition`) before applying its own, because CSS specificity resolves *per property*, not per rule: a more specific selector that omits a property doesn't block a less specific rule from supplying it. Checked systematically afterward with a script comparing every class the new tab uses against every bare (unscoped) selector in the stylesheet — the remaining matches (`.tab-panel.active`, `.subnav`, and a `@media print` rule) are intentional reuse or print-only, not bugs |
